...
def result_string(array):
    result= f'{array[0]}'

    for i in range (1,len(array)):
        add_space = True
        if array[i-1] == '"' and is_digit(array[i]) != -1 :
            add_space = False
        if array[i-1] == '"' and is_digit_with_sign(array[i]) != -1 :
            add_space = False
        if array[i] == '"' and is_digit(array[i-1]) != -1 :
            add_space = False
        if array[i] == '"' and is_digit_with_sign(array[i-1]) != -1 :
            add_space = False
...
#  here we will enter ' "" '  to new list

new_list = []
for i in range(len(list1)):
    if is_digit(list1[i]) != -1 or is_digit_with_sign(list1[i]) != -1:
        new_list.append('"')
